...
class DisposisiResource extends Resource
{
    ...
    {
        return auth()->user()?->hasAnyRole(['admin', 'pimpinan', 'sekretaris', 'staf']);
    }
    ...
    public static function form(Schema $form): Schema
    {
        return $form
            ->schema([
                Section::make('Informasi Disposisi')
                    ->schema([
        ...
        return $table
            ...
            ->actions([
                \Filament\Actions\Action::make('prosesDisposisi')
                    ...
                    ->visible(fn($record) => $record->status === 'belum_diproses'),
                \Filament\Actions\Action::make('selesaiDisposisi')
                    ...
                    ->action(function (Disposisi $record) {
                        ...
                        Notification::make()
                            ...
                            ->body("Disposisi untuk surat {$record->suratMasuk->perihal} telah selesai diproses")
                            ->icon('heroicon-o-check-circle')
                            ->iconColor('success')
                            ->sendToDatabase($record->dariUser);

                        Notification::make()
                            ->title('Disposisi ditandai selesai')
                            ->success()
                            ->send();
                    })
                    ->visible(fn($record) => $record->status === 'sedang_diproses'),
                \Filament\Actions\Action::make('teruskanDisposisi')
                    ...
                    ->action(function (Disposisi $record, array $data) {
                        ...
                        Notification::make()
                            ->title('Disposisi berhasil diteruskan')
                            ->success()
                            ->send();
                    })
